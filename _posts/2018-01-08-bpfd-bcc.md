...
C program is being compiled, found in `src/cc/export/helpers.h` in the BCC sources. This `helpers.h` header uses
the `LINUX_VERSION_CODE` macro to create a "version" section in the compiled output. `LINUX_VERSION_CODE` is available
only in the specific kernel's sources being targetted and is used during eBPF program loading to make sure the BPF
program is being loaded into a kernel with the right version. As you can see, kernel sources quickly become mandatory
...
If you have any questions, feel to [reach out](http://www.linuxinternals.org/joel/) to me or drop me a note in the comments seciton.
