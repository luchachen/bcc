...
parts of BCC that make calls to `libbpf.c` are now instead channeled to the
...
but certain other paths also need to be remotely channeled. For example to
attach to a tracepoint, BCC needs a list of all available tracepoints on the
system. This list has to be that of the remote system, not the local one.
The [GET_TRACE_EVENTS](https://github.com/joelagnel/bpfd/blob/master/src/bpfd.c#L421)
in `BPFd` is exactly for this reason.


- Making the kernel build more configurable so that kernel source path and target architecture information can be passed
  with environment variables to BCC.
