...
[BCC tools](https://github.com/iovisor/bcc/blob/master/README.md) are a suite of kernel tracing tools that allow systems engineers to efficiently and safely get a deep understanding into the inner workings of a Linux system. Because they can't crash the kernel, they are safer than kernel modules and also can be used in production. [Brendan Gregg has written](http://www.brendangregg.com/ebpf.html) several nice tools, and has given talks showing the full power of eBPF based tools.

In the Android kernel team, we work mostly on ARM64 systems, since most Android devices are on this architecture. BCC
tools support on ARM64 systems has stayed broken for years. One of the reasons for this difficulty is with ARM64 inline assembler statements. Unavoidably, kernel header includes in BCC tools result in inclusion of asm headers which in the case of ARM64 has the potential of spewing inline asm ARM64 instructions [causing major pains](https://www.mail-archive.com/iovisor-dev@lists.iovisor.org/msg00299.html)  to LLVM's BPF backend.

 which has been the subject of several threads. Recently this issue got fixed by BPF inline asm support (these [LLVM](https://github.com/llvm-mirror/llvm/commit/2865ab6996164e7854d55c9e21c065fad7c26569) [commits]( https://github.com/llvm-mirror/llvm/commit/a6b7d22c2e64f44e6c74ad7e5ce5670f5ae72da3)) and [folks could finally run BCC tools on arm64]((https://github.com/iovisor/bcc/issues/1202)), but..
...
communication methods such as USB or Networking. As a part of my performance analysis work in the Android kernel team, I am
going to be communicating these commands over the [Android Debug
Bridge](https://developer.android.com/studio/command-line/adb.html) which interfaces with the target device over either USB or TCP/IP.
...
While the main usecase at the moment is easier use of BCC tools on cross-development models, another potential usecase that's gaining interest is to design an easy to use BPF program loader. The BPFd code can be stored on disk in base64 format and sent to bpfd using something as simple as:
```
joel@ubuntu:~/bpfprogs# cat my_bpf_prog.base64 | bpfd
```

### Conclusion and a word on similiarity of BPFd to SystemTap:
Building code for instrumentation on a different machine than the one actually running the debugging code is beneficial and BPFd makes this possible. Tools like
...
Feel to write to me on [twitter](https://twitter.com/joel_linux) or [visit my blog](http://www.linuxinternals.org/)  for more.
