...
```
In the Android kernel team, we are also expermenting for certain usecases that need eBPF, with loading a program with a forked BPFd instance, creating maps, and then pinning them for use at a later time once BPFd exits and then exiting BPFd.
...
If you have any questions, feel to write to me on [twitter](https://twitter.com/joel_linux) or [visit my blog](http://www.linuxinternals.org/).
