...
While most issues have been fixed, a few remain. Patches and testing are welcomed!
Following is an incomplete list:
- cachetop doesn't show hits/misses counter on arm64
- syscount reads `comm_for_pid` locally, instead of on the remote.
- Issues with stacktrace based tools:
  - offcputime on arm64 shows "unknown" from the stack trace.
  - getting tonnes of stack traces is still slow - possible fix is to dump entire map remotely and send it at once instead of individually sending each key/value as a separate command.
